...
namespace SendDataContinuouslySeparateThread{
...
public:
	...
	void Run(){
		...
		//Accept some connection
		TRACE(<< "ServerThread::Run(): accepting connection" << std::endl)
		ting::TCPSocket sockR;
		for(unsigned i = 0; i < 20 && sockR.IsNotValid(); ++i){
			ting::Thread::Sleep(100);
			sockR = serverSock.Accept();
		...
		ASSERT_ALWAYS(sockR.IsValid())
		TRACE(<< "ServerThread::Run(): connection accepted" << std::endl)
		...
		while(!this->quitFlag){
			...
			for(unsigned i = 0; i < numTriggered; ++i){
				if(triggered[i] == &this->queue){
					ASSERT_ALWAYS(triggered[i] != &sockR)
					TRACE(<< "SendDataContinuously::Run(): queue triggered" << std::endl)
					this->queue.GetMsg()->Handle();
				}else if(triggered[i] == &sockR){
					ASSERT_ALWAYS(triggered[i] != &this->queue)
...
void Run(){
	...
	ting::Thread::Sleep(100);//give thread time to start accepting
	...
	while(ting::GetTicks() - startTime < 10000){
		unsigned numTriggered = ws.WaitWithTimeout(1000);
//		unsigned numTriggered = ws.Wait(&triggered);
...
//	TRACE_ALWAYS(<<"Socket test "<<std::endl)
	...
	//SendDataContinuouslySeparateThread::Run();
